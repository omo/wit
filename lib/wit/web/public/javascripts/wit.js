...
WIT.pageLabelFrom = function(url) {
  var m = url.match(/\+\/((\w|\d)+)$/);
  if (!m)
    return null;
  return m[1];
...
WIT.Editor = function() {
  ...
  this.titlePageSignEl = $(".edit-title-page-sign");
  ...
  this.shortcut.add("X", true, this.cancelEditing.bind(this));
...
WIT.Editor.prototype.didGetNoteURL = function(noteURL) {
  ...
  this.fillPageTitleIfNeeded(noteURL);
};

WIT.Editor.prototype.fillPageTitleIfNeeded = function(noteURL) {
  var pageLabel = WIT.pageLabelFrom(noteURL);

  if (!pageLabel) {
    this.titlePageSignEl.addClass("edit-title-page-sign-disabled");
    return;
  }

  this.titlePageSignEl.removeClass("edit-title-page-sign-disabled");
  var title = this.titleEl.val();
  if (0 == title.length)
    this.titleEl.val(pageLabel);
};

WIT.Editor.prototype.cancelEditing = function() {
  if (this.noteLinkEl.attr("href") != WIT.THINK_URL)
    this.moveToNote();
  else
    window.history.back();
};
...
WIT.NoteView = function() {
  ...
  this.shortcut.add("U", false, this.moveToRef.bind(this, this.upLinkEl));
};
...
// PageIndexView
//
WIT.PageIndexView = function() {
  this.labelEl = $("#newPageLabel");
  this.buttonEl = $("#newPageButton");
  this.buttonEl.on("click", this.onNewPage.bind(this));
};

WIT.PageIndexView.prototype.onNewPage = function() {
  var label = this.labelEl.val();
  if (0 == label.length) {
    WIT.tell("Label is empty.");
    return;
  }

  if (!label.match(/^(\w|\d)+$/g)) {
    WIT.tell("Invalid label.");
    return;
  }

  var dest = WIT.EDIT_URL + "/+/" + label;
  window.location = dest;
}
...
WIT.initPageIndexView = function() {
  WIT.pageIndexView = new WIT.PageIndexView();
};